...
PDF
 │
 ├─→ pdf-parse (텍스트) → Text Chunking → Embeddings
 │                                              │
 │                                              ↓
 └─→ Page Rendering → Vision AI (Figure 감지/분석)
                           │
                           ↓
                      Figure Chunks
                           │
                           ↓
                     paper_chunks (통합) → pgvector → Hybrid Search → LLM
                           │                                          │
                           ↓                                          ↓
                   Supabase Storage ←──────────────────────── Figure 인라인 표시
                   (Figure 이미지)
```
...
│   └─→ Return answer + groundingChunks (with figure image URLs)              │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
-- 4. 기존 unique constraint 업데이트 (figure는 chunk_index 대신 figure_number 사용)
-- 기존: UNIQUE (paper_id, collection_id, chunk_index)
-- Figure의 경우 figure_number로 중복 방지

-- 5. RLS 정책 업데이트 (기존 정책 유지, figure 타입도 동일하게 적용)
```
...
├── chunk_type VARCHAR(20) ['text' | 'figure']  ← NEW
│
├── [Text Chunk Fields]
│   ├── content TEXT
│   ├── chunk_index INT
...
### 5.1 Gemini Vision을 이용한 Figure 감지
...
export async function analyzeFigure(
  ...
  const prompt = FIGURE_ANALYSIS_PROMPT.replace(
    '{figureNumber}',
...
export async function analyzeAllFigures(
  ...
  for (let i = 0; i < figures.length; i += CONCURRENT_ANALYSIS) {
    ...
    const batchResults = await Promise.all(
      batch.map(fig => analyzeFigure(fig))
    );
...
export async function indexFigures(
  ...
  const storagePaths = await Promise.all(
    figures.map(fig =>
      uploadFigureImage(fig.imageBuffer, fig.paperId, fig.figureNumber)
    )
  );
  ...
  const records = figures.map((fig, i) => ({
    ...
    chunk_type: 'figure',
    content: contents[i],
    chunk_index: i, // Figure는 순서가 덜 중요하므로 0부터
...
RETURNS TABLE (
  ...
  content TEXT,
  chunk_index INT,
  chunk_type VARCHAR(20),          -- NEW
  figure_number VARCHAR(20),        -- NEW
  figure_caption TEXT,              -- NEW
  figure_description TEXT,          -- NEW
  image_storage_path VARCHAR(500),  -- NEW
  page_number INT,                  -- NEW
...
export interface SearchResult {
  ...
  content: string;
  chunkIndex: number;
  chunkType: 'text' | 'figure'; // NEW
  figureNumber?: string; // NEW
  figureCaption?: string; // NEW
  figureDescription?: string; // NEW
  imageStoragePath?: string; // NEW
  ...
  return chunks
    .map((c, i) => {
      if (c.chunkType === 'figure') {
        // Figure chunk
        return `[${i + 1}] [FIGURE: ${c.figureNumber}] (Paper ID: ${c.paperId}, Page ${c.pageNumber})
Caption: ${c.figureCaption || 'No caption'}

${c.figureDescription}`;
      } else {
        // Text chunk (기존)
        return `[${i + 1}] (Paper ID: ${c.paperId})
${c.content}`;
      }
    })
    .join('\n\n---\n\n');
...
1. Use [CITE:N] markers to cite specific text sources
2. Use [FIGURE:N] markers to reference figures (e.g., "[FIGURE:Figure 1] shows the architecture...")
...
- When referencing a figure, explain what it depicts
...
function parseCitations(text: string, chunks: SearchResult[]): ParsedCitations {
  const citedIndices: number[] = [];
  const figureReferences: { index: number; figureNumber: string }[] = [];
  ...
  while ((match = figureRegex.exec(text)) !== null) {
    ...
    if (chunkIdx >= 0 && !citedIndices.includes(chunkIdx)) {
      citedIndices.push(chunkIdx);
      figureReferences.push({ index: chunkIdx, figureNumber });
    }
  }
  ...
  try {
    ...
    for (const page of pages) {
      ...
      for (const figure of analysis.figures) {
        ...
        allFigures.push(cropped);
      }
    }

    // 7. [NEW] Analyze figures with Vision AI
    await job.updateProgress(70);
    const analyzedFigures = await analyzeAllFigures(allFigures);
...
### Phase 2: PDF 렌더링 및 Figure 감지 (3일)

4. `src/lib/pdf/renderer.ts` 구현
5. `src/lib/pdf/figure-detector.ts` 구현
6. `src/lib/pdf/figure-extractor.ts` 구현
7. 단위 테스트 작성 및 검증

### Phase 3: Figure 분석 및 인덱싱 (3일)

8. `src/lib/pdf/figure-analyzer.ts` 구현
9. `src/lib/pdf/figure-pipeline.ts` 구현
10. `src/lib/storage/figures.ts` 구현
11. `src/lib/rag/figure-indexer.ts` 구현

### Phase 4: 검색 및 RAG 수정 (2일)

12. `hybrid_search` 함수 업데이트
13. `src/lib/rag/search.ts` 타입 확장
14. `src/lib/rag/index.ts` 컨텍스트 빌드 및 파싱 수정

### Phase 5: Worker 통합 (2일)

15. `pdfIndexWorker.ts` 확장
16. 워커 테스트 (단일 PDF로 전체 파이프라인 검증)

### Phase 6: UI 구현 (2일)

17. `FigureInline.tsx` 컴포넌트 구현
18. 메시지 렌더링 로직 수정
19. E2E 테스트

### Phase 7: 최적화 및 안정화 (2일)

20. 메모리 사용량 최적화 (대용량 PDF)
21. 에러 핸들링 강화
22. 성능 테스트 및 튜닝
23. 문서 업데이트
